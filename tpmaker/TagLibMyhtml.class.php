    ...
    public function _editlist($attr)
    {
        ...
        foreach($fields as $field) {//显示指定的字段
           ...
           // dump($property);
            ...
            $parseStr .= '<A HREF="javascript:sortBy(\''.$trimtalbename.'\',\'{:getsorttype(\''.$trimtalbename.'\')}\',\''.ACTION_NAME.'\')" title="按照'.$showname[2].'排序">'.$titlename[0].'{:getsorttype(\''.$trimtalbename.'\',1)}</A>';
			}
			$parseStr .= '</Th>';
