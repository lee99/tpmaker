    ...
    function __init()
    {
        ...
        for (var _loc2 = 0; _loc2 < __config.graphs.graph.length; ++_loc2)
        {
            if (__config.graphs.graph[_loc2].visible_in_legend == true)
            {
                ...
                var _loc11 = new com.amcharts.Rectangle(__legend_entry_mc[_loc2], "rectangle", 0, __key_size, __key_size, __config.graphs.graph[_loc2].color, 0, __config.legend.key.border_color, 0, 100, _loc7);
                if (__config.graphs.graph[_loc2].pattern != undefined)
                {
                    var _loc6 = new com.amcharts.Rectangle(__legend_entry_mc[_loc2], "mask_mc", 5, __key_size, __key_size, 0, 0, 0, 0, 100).__get__mc();
                    _loc6._alpha = 0;
                    var _loc5 = __legend_entry_mc[_loc2].createEmptyMovieClip("pattern_mc", 3);
                    var _loc8 = _loc5.createEmptyMovieClip("pattern_loader_mc", 0);
                    _loc5.setMask(_loc6);
                    if (__config.graphs.graph[_loc2].pattern_color != undefined)
                    {
                        var _loc9 = new Color(_loc5);
                        _loc9.setRGB(__config.graphs.graph[_loc2].pattern_color);
                    } // end if
                    var _loc10 = new com.amcharts.Load();
                    _loc10.loadClip(__config.path + __config.graphs.graph[_loc2].pattern, _loc8);
                } // end if
