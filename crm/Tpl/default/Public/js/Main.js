...
function displaceSelect(DispalceID, HiddenInputID, FromID)
{
    ...
    {
        addItem($(FromID), $(DispalceID));
    }
    else
    {
        delItem($(DispalceID));
    }
    $(HiddenInputID).value=joinItem($(DispalceID));
}
...
function selectDiv(inputObj)
{
    ...
    this.createContainerDiv = function()
    {
        ...
        {
            insertAfter(inputObj.id,'div',this.id);
        }
        this.ContainerDiv = $(this.id);
    ...
    this.createOptionDiv = function(optionKeyList,optionValueList)
    {
        ...
        else
        {
            for(i=0; i< optionKeyList.length; i++)
            {
                ...
                if(!$(optionId))
                {
                    create(this.id,'div',optionId);
                }
                $(optionId).innerHTML = optionValueList[i];
                $(optionId).style.textAlign = 'left';
                $(optionId).style.padding = '2px 2px 0';
                $(optionId).key = optionKeyList[i];
                $(optionId).onclick = function()
                {
                    eval(inputObj.setValueFunction+"('"+inputObj.id+"','"+this.key+"');");
                }
                $(optionId).onmouseover = function()
                {
                    unselectAllOption($(this.id).parentNode.id);
                    selectDivSelectOption(this.id);
                }
                $(optionId).onmouseout = function()
                {
    ...
    {
        return $(this.id+'Sub_'+this.selectedOptionNum).key;
    }

    this.getSelectedValue = function()
    {
        return $(this.id+'Sub_'+this.selectedOptionNum).innerHTML.toString();
    }
...
{
    var options = $(objId).getElementsByTagName('div');
...
{
    try{
    $(optionId).style.color = '#FFFFFF';
    //$(optionId).style.fontWeight = 'bold';
    $(optionId).style.backgroundColor = '#37428A';
    $(optionId).selected = 1;
    }catch(e){};
}
...
{
    try{
    $(optionId).style.color = '#000000';
    //$(optionId).style.fontWeight = 'normal';
    $(optionId).style.backgroundColor = '#F1F1F1';
    $(optionId).selected = 0;
    }catch(e){};
}
        ...
        else
        {
            ...
            if(eventElement.reLoadData != 1 && typeof(eventElement.selectDiv) != "undefined" && typeof(eventElement.selectDiv.optionKeyList) != "undefined" && eventElement.selectDiv.optionKeyList != "")
            {
                ...
                else
                {
                    ...
                    eventElement.searchValue = newSearchValue;
                    xajax_xCreateSelectDiv(eId,eventElement.searchValue, eventElement.loadFunction, $('CurrentProjectID').value);
                }
            }
            else
            {
...
function initSelectDiv(objId,loadFunction,getSearchValueFunction,setValueFunction,reLoadData)
{
    ...
    addHandler(objId,'onkeyup','showDivList');
...
function hiddenObj(objId)
{
    ...
    {
        $(objIdList[i]).style.display = "none";
    }
}
...
function displayObj(objId)
{
    ...
    for(i=0;i<objIdList.length;i++)
    {
        $(objIdList[i]).style.display = "block";
    }
}
...
function disableObj(objId)
{
    ...
    for(i=0;i<objIdList.length;i++)
    {
        $(objIdList[i]).style.disabled = "disabled";
    }
}
...
function setQueryOperator(I)
{
    ...
    for(j=0;j<FieldType.length;j++)
    {
        ...
        {
            delAllItem($("Operator"+I));
            addOptionByTextValue($("Operator"+I), FieldOperationTypeText[j].split(','),FieldOperationTypeValue[j].split(','));
            break;
        }
    }
    ...
    //{
    //    selectOneItem($("Operator" + I),"LIKE");
    //}
    //else
    //{
    //    selectOneItem($("Operator" + I),"=");
    //}
}
...
function setQueryValue(I, Reset)
{
    ...
    if (FieldName.match("ProjectName") && ($('Operator'+I).value == '=' || $('Operator'+I).value == '!='))
    {
